...
[[date-match-index-names]]
== Date match support in index names

Date match index name resolution enables you to search a range of time-series indices, rather
...
daily logs, you can use a date match name template to restrict the search to the past
two days.

Almost all APIs that have an `index` parameter, support date match in the `index` parameter
value.

A date match index name takes the following form:

[source,txt]
----------------------------------------------------------------------
<static_name{date_match_expr{date_format|time_zone}}>
----------------------------------------------------------------------
